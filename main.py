...
class DatabaseManager:
    ...
    def insert_sample_data(self, conn):
        ...
        cursor.executemany('INSERT OR IGNORE INTO faculty VALUES (?, ?, ?, ?, ?)', faculty_data)
    ...
    def get_next_id(self, table_name, id_column):
        ...
        result = self.execute_query(f"SELECT MAX({id_column}) FROM {table_name}")
        max_id = result[0][0] if result[0][0] is not None else 0
        return max_id + 1
    
    def delete_student(self, student_id):
        self.execute_query("DELETE FROM students WHERE student_id = ?", (student_id,))
    
    def delete_faculty(self, faculty_id):
        self.execute_query("DELETE FROM faculty WHERE faculty_id = ?", (faculty_id,))
    
    def delete_course(self, course_id):
        self.execute_query("DELETE FROM courses WHERE course_id = ?", (course_id,))
    ...
    def generate_timetable(self):
        ...
        for course in courses:
            ...
            timetable_data.append((course_id, faculty_id, room_id, day, time_slot))
...
class CampusManagementApp:
    ...
    def setup_dashboard(self):
        ...
        # Quick stats frame
        stats_frame = ttk.LabelFrame(self.dashboard_frame, text="Quick Statistics", padding=15)
        stats_frame.pack(fill='x', padx=50, pady=20)
        
        # Stats labels
        self.students_count_label = ttk.Label(stats_frame, text="Students: Loading...", font=('Arial', 12))
        self.students_count_label.grid(row=0, column=0, padx=20, pady=10, sticky='w')
        
        self.faculty_count_label = ttk.Label(stats_frame, text="Faculty: Loading...", font=('Arial', 12))
        self.faculty_count_label.grid(row=0, column=1, padx=20, pady=10, sticky='w')
        
        self.courses_count_label = ttk.Label(stats_frame, text="Courses: Loading...", font=('Arial', 12))
        self.courses_count_label.grid(row=1, column=0, padx=20, pady=10, sticky='w')
        
        self.rooms_count_label = ttk.Label(stats_frame, text="Rooms: Loading...", font=('Arial', 12))
        self.rooms_count_label.grid(row=1, column=1, padx=20, pady=10, sticky='w')
    ...
    def setup_faculty_tab(self):
        ...
        self.faculty_tree = ttk.Treeview(main_frame, columns=columns, show='headings', height=15)
        for col in columns:
            self.faculty_tree.heading(col, text=col)
            self.faculty_tree.column(col, width=120)
        
        scrollbar = ttk.Scrollbar(main_frame, orient='vertical', command=self.faculty_tree.yview)
    ...
    def setup_courses_tab(self):
        ...
        self.courses_tree = ttk.Treeview(main_frame, columns=columns, show='headings', height=15)
        for col in columns:
            self.courses_tree.heading(col, text=col)
            self.courses_tree.column(col, width=100)
        
        scrollbar = ttk.Scrollbar(main_frame, orient='vertical', command=self.courses_tree.yview)
    ...
    def setup_rooms_tab(self):
        ...
        self.rooms_tree = ttk.Treeview(main_frame, columns=columns, show='headings', height=15)
        for col in columns:
            self.rooms_tree.heading(col, text=col)
            self.rooms_tree.column(col, width=120)
        
        scrollbar = ttk.Scrollbar(main_frame, orient='vertical', command=self.rooms_tree.yview)
    ...
    def setup_timetable_tab(self):
        ...
        # Timetable treeview
        columns = ('Course Code', 'Course Name', 'Faculty', 'Room', 'Day', 'Time Slot')
        self.timetable_tree = ttk.Treeview(main_frame, columns=columns, show='headings', height=20)
        for col in columns:
            self.timetable_tree.heading(col, text=col)
            self.timetable_tree.column(col, width=150)
        
        scrollbar = ttk.Scrollbar(main_frame, orient='vertical', command=self.timetable_tree.yview)
    ...
    def load_timetable(self):
        ...
        for t in timetable:
            self.timetable_tree.insert('', 'end', values=t[1:])  # Skip timetable_id
    ...
    def add_faculty(self):
        ...
        ttk.Button(dialog, text="Save", command=save_faculty).grid(row=len(fields), column=0, columnspan=2, pady=10)
        dialog.columnconfigure(1, weight=1)
    ...
    def add_course(self):
        ...
        for i, (field, label, default) in enumerate(fields):
            ...
            if field == 'faculty_id':
                ...
                faculty_names = [f"{fid} - {name}" for fid, name in faculty_dict.items()]
                combo = ttk.Combobox(dialog, values=faculty_names, state='readonly')
        ...
        ttk.Button(dialog, text="Save", command=save_course).grid(row=len(fields), column=0, columnspan=2, pady=10)
        dialog.columnconfigure(1, weight=1)
    ...
    def add_room(self):
        ...
        ttk.Button(dialog, text="Save", command=save_room).grid(row=len(fields), column=0, columnspan=2, pady=10)
        dialog.columnconfigure(1, weight=1)
    ...
    def generate_timetable(self):
        ...
        try:
            ...
            self.load_timetable()
            messagebox.showinfo("Success", f"Timetable generated successfully with {count} entries!")
        except Exception as e:
